...
describe('vllmProvider', () => {
  ...
  it('includes reasoning_format and extra_body when vllm.enable_reasoning is on', async () => {
    mockGetSetting.mockImplementation((key) => {
      if (key === 'vllm.base_url') return 'http://vllm:8000'
      if (key === 'api_key.vllm') return 'key'
  ...
  it('includes reasoning_format and extra_body when vllm.enable_reasoning is off', async () => {
    mockGetSetting.mockImplementation((key) => {
      if (key === 'vllm.base_url') return 'http://vllm:8000'
      if (key === 'api_key.vllm') return 'key'
  ...
  it('uses reasoning_format=auto when vllm.enable_reasoning is undefined', async () => {
    mockGetSetting.mockImplementation((key) => {
      if (key === 'vllm.base_url') return 'http://vllm:8000'
      if (key === 'api_key.vllm') return 'key'
      return undefined
